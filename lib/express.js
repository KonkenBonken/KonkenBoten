...
export default function setup({ client, DataBase, OAUTH, Discord: { BitField } }) {
	...
	app.use((req, res, next) => {
		...
		try {
			...
			if (!['static', 'manifest.json', 'icon.svg'].includes(req.path.split('/')[1]))
	...
	app.use('/static', express.static('./build/site/static', {
		immutable: true,
		maxAge: '1y'
	}));
	...
	io.on('connection', async socket => {
				...
				discord: {
					categories: guild.channels.cache.filter(channel => channel.type == 'GUILD_CATEGORY')
						...
						.map((category, i) => ({
							id: category.id,
							name: category.name,
							position: Default(category.position, i)
						})),
					channels: guild.channels.cache.filter(channel => !channel.isThread() && channel.type != 'GUILD_CATEGORY')
						...
						.map((channel, i) => ({
							id: channel.id,
							name: channel.name,
							position: Default(channel.rawPosition, i),
							type: Default(channel.type.replace('GUILD_', ''), 'TEXT'),
							parent: channel.parentId,
							permissions: Default(channel.permissionsFor(guild.me).bitfield.toString(), "2199023255551")
						})),
					members: guild.members.cache
						.map(member => ({
							...
							color: Default(member.displayHexColor, '#000000'),
					...
					roles: guild.roles.cache
						...
						.map((role, i) => ({
							id: role.id,
							name: role.name,
							position: Default(role.position, i),
							color: Default(role.hexColor, '#000000'),
		...
		socket.on('changes', async (data, response) => {
			console.log(data);
		});
	});
