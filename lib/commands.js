...
export default (globals) => {
	...
	return commands = [{
			...
			default: true,
			options: {
				message: { type: 'STRING', description: 'The message you want to send; max 4000 characters', required: true }
			},
		...
		format: Object.entries(command.options)
			.map(([name, opt]) =>
				newDiv('span', 'argel')
				.Attribute('arg', (requried ? '?' : '') + name)
				.Append(
					newDiv('span')
					.Html([
							({
