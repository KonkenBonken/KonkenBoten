...
async function afterMod({ com, Moderation, guild, author, user, reason, banMessage, dmInvite, duration }) {
	const until = duration && new Date(Date.now() + duration * 1000),
		comType = com.replace(/^temp/, '');

	Moderation.logs.push({ t: comType, s: author.id, r: reason, m: user.id, d: encodeT(), dur: duration, unt: until && encodeT(until) });
	...
	if (dmInvite) {
		...
		var dmInvite =
			invites && [...invites.values()].find(i => i.inviter?.id == guild.me.id && i.channel.id == channel.id) ||
			await channel.createInvite({ maxAge: 0, unique: true, reason: 'Created an Invite for inviting tempbanned or kicked members' });
	...
	await user.send({
		content: dmInvite,
		embeds: [{
				author: { name: `${user.tag} ${text.hasBeen} ${text.txt} ${Moderation.tellWho? `${text.by} ${staff?.user?.tag}`:''}` },
				description: reason,
			...
			banMessage && Moderation.dmAll && Moderation.banMessage && {
				...
				description: banMessage && Moderation.dmAll && Moderation.banMessage,
				color: parseInt(text.color, 16) || 14396689
			}
		].filter(x => x)
	});
...
let commands;
let client, Duration, CleanDate, moment, ParseModLogs, WriteDataBase, ObjectMerge, MutedPermissions, DataBase, capital, newDiv;
export default (globals) => {
	...
	return commands = [{
			...
			handler: ({ message, private: priv }, { channel, reply }) => {
				...
				return channel.send({ content, ephemeral: !!priv }).then(() =>
			...
			handler: async ({ duration, seconds, private: priv }, { channel, user, reply }, { error }) => {
			...
			handler: async ({ amount, private: priv }, { channel, reply, member }, { error }) => {
...

		{
			...
			handler: async ({ user, private: priv }, { guild, reply }, { error }) => {
			...
			handler: async ({ private: priv }, { guild, reply }, { GuildData }) => {
				const [channels, { threads }] = await Promise.all([
						guild.channels.fetch(),
						guild.channels.fetchActiveThreads(),
						guild.member.fetch({ time: 10e3 })
					]),
					botCount = guild.member.cache.filter(m => m.user.bot).size;
			...
			handler: async ({ role, private: priv }, { member, user: author, reply, guild }) => {
...

		{
			...
			handler: async ({ user, amount }, { reply, guild, member: author }, { GuildData }) => {
			...
			handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
				if (!GuildData.Moderation) GuildData.Moderation = {};
				const { Moderation } = GuildData;

				const member = await guild.members.fetch(user).catch(e => false),
					modChannel = await client.channels.fetch(Moderation.channel).catch(e => false);

				if (!member) return error('User not found');
				if (!modChannel) return error('Channel not found');
				if (guild.ownerId === member.id) return error('Cannot warn server owner');
				if (author.roles.highest.comparePositionTo(member.roles.highest) <= 0) return error('Cannot warn members with a higher role then you');
				if (member.roles.highest.comparePositionTo(guild.me.roles.highest) <= 0) return error('<@813803575264018433> cannot warn members with higher role then itself');

				afterMod({ com: 'warn', Moderation, guild, author, user, reason });
				reply({ content: `${member.displayName} has been warned`, ephemeral: true });
			}
		}, {
			com: 'kick',
			...
			handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
				...
				afterMod({ com: 'kick', Moderation, guild, author, user, reason, dmInvite: true });
				reply({ content: `${member.displayName} has been kicked`, ephemeral: true });
			}
		}, {
			com: 'ban',
			...
			handler: async ({ user, reason, duration }, { reply, guild, member: author }, { GuildData }) => {
				if (!GuildData.Moderation) GuildData.Moderation = {};
				const { Moderation } = GuildData;
				...
				if (duration) {
					...
					DataBase.temp[tempIndex] = { g: guild.id, m: user.id, type: 'ban', until: +until };
				...
				afterMod({ com: duration ? 'tempban' : 'ban', Moderation, guild, author, user, reason, dmInvite: !!duration, duration });
				reply({ content: `${member?.displayName || user.username} has been banned`, ephemeral: true });
			}
		}, {
			com: 'unban',
			...
			handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
				...
				afterMod({ com: 'unban', Moderation, guild, author, user, reason, dmInvite: true });
				reply({ content: `${member?.displayName || user.username} has been unbanned`, ephemeral: true });
			}
		}, {
			com: 'mute',
			...
			handler: async ({ user, reason, duration }, { reply, guild, member: author }, { GuildData }) => {
				...
				if (Moderation.timeout)
					await member.timeout(Math.min(duration * 1000, 24192e5) || 24192e5, reason); //28d
				else {
					...
					if (!mutedRole) {
						mutedRole = await guild.roles.create({
							...
							position: guild.roles.botRoleFor(client.user).position,
							mentionable: false,
					...
					if (guild.me.roles.highest.comparePositionTo(mutedRole) <= 0) return error(`The role <@&${mutedRole.id}> is higher than <@813803575264018433>`);

					await member.roles.add(mutedRole, reason);

					if (duration) {
						let tempIndex = DataBase.temp.length;
						DataBase.temp[tempIndex] = { g: guild.id, m: member.id, type: 'mute', until: +until, role: muted, key: [staff.id, d, (reason || '*').replace(/\W/g, '').substr(0, 20)].join('-') };

						if (duration <= 2147483) setTimeout(() => {
								member.roles.remove(muted, 'Temporarily mute expired')
									.then(() => delete DataBase.temp[tempIndex]);
							},
							duration * 1000);
					}
				...
				afterMod({ com: duration ? 'tempmute' : 'mute', Moderation, guild, author, user, reason, duration });
				reply({ content: `${member.displayName} has been warned`, ephemeral: true });
			}
		}, {
			com: 'unmute',
			...
			handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
				...
				afterMod({ com: 'unmute', Moderation, guild, author, user, reason });
				reply({ content: `${member.displayName} has been warned`, ephemeral: true });
			}
		}
	].map((command) => ({
			...
			.map(([name, opt]) =>
				...
				.Append(
					...
					.Html([
							...
							opt.description
							.replace('%d-d', 'possible units: h, m, s; ex: 2m30s')
