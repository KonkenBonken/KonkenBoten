...
export async function modArgs({ Moderation, guild, member, modChannel, reason, author, duration, dmInvite = false }) {
	if (!Moderation.coms) Moderation.coms = {};

	if (Moderation.logsEnabled) {
		if (!Moderation.logs) Moderation.logs = {};
		if (!Moderation.logs[author.user.id]) Moderation.logs[author.user.id] = [];
	}

	if (dmInvite && Moderation.dmInvite) {
		let invites = await m.guild.invites.fetch().catch(e => undefined);
		var dmInvite =
	...
	return {
		...
		channel: modChannel, //Channel
		reason: reason?.trim().substr(0, 512) || undefined, //?string
		guild, //Guild
		staff: author, //Member
		logs: Moderation.logsEnabled ? Moderation.logs[author.id] : [], //array
		muted: Moderation.muted, //id
		duration, //seconds
		until: duration && new Date(Date.now() + duration * 1000), //Date
		text: {
			...
			...ObjectMerge(
				commands.find(c => c.com == modCommand) || {},
				Moderation.coms?.[modCommand] || {}
			)
		},
		dmInvite, //string
		banMessage: Moderation.banMessage, //string
		dmAll: !!Moderation.dmAll, //bool
		tellWho: !!Moderation.tellWho, //bool
		timeout: !!Moderation.timeout && !!member.timeout //bool
	}
}

export const commands = ({ client, Duration, CleanDate, capital, moment, ParseModLogs, WriteDataBase, ObjectMerge, MutedPermissions /* DataBase, Cache*/ }) => [{
		com: 'suggest',
...

	{
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			const args = modArgs({ Moderation, guild, member, modChannel, reason, author });
			await moderationCommands.warn(args);
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			const args = modArgs({ Moderation, guild, member, modChannel, reason, author, dmInvite: true });
			await moderationCommands.kick(args);
		...
		handler: async ({ user, reason, duration }, { reply, guild, member: author }, { GuildData }) => {
			...
			const args = modArgs({ Moderation, guild, member: member || user.id, modChannel, reason, author, duration, dmInvite: !!duration });
			await moderationCommands.ban(args);
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			const args = modArgs({ Moderation, guild, member: member || user.id, modChannel, reason, author, dmInvite: true });
			await moderationCommands.unban(args);
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			if (author.roles.highest.comparePositionTo(member.roles.highest) <= 0) return error('Cannot warn members with a higher role then you');
			if (member.roles.highest.comparePositionTo(guild.me.roles.highest) <= 0) return error('<@813803575264018433> cannot warn members with higher role then itself');
			if (guild.me.roles.highest.comparePositionTo(mutedRole) <= 0) return error();

			try {
				if (duration) duration = Duration(duration);
			} catch { return error('Invalid duration') }

			const args = modArgs({ Moderation, guild, member, modChannel, reason, author, duration });
			await moderationCommands.mute(args);
			reply({ content: `${member.displayName} has been warned`, ephemeral: true });
		}
	}, {
		com: 'unmute',
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			const args = modArgs({ Moderation, guild, member, modChannel, reason, author, duration });
			await moderationCommands.unmute(args);
			reply({ content: `${member.displayName} has been warned`, ephemeral: true });
		}
	}
];
