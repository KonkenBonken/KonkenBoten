...
async function afterMod({ com, GuildData, guild, author, user, reason, banMessage, dmInvite, duration, allowNoMember }, fun) {
	...
	return commands = [{
...

		{
			...
			handler: async ({ user, reason }, { guild, member: author }, { GuildData, error, reply }) =>
				afterMod({ com: 'warn', GuildData, guild, author, user, reason }).then(() =>
			...
			handler: async ({ user, reason }, { guild, member: author }, { GuildData, reply, error }) =>
				afterMod({ com: 'kick', GuildData, guild, author, user, reason, dmInvite: true },
			...
			handler: async ({ user, reason, duration }, { guild, member: author }, { GuildData, reply, error }) =>
				afterMod({ com: duration ? 'tempban' : 'ban', GuildData, guild, author, user, reason, dmInvite: !!duration, duration, allowNoMember: true },
					async () => {
						try {
							if (duration) duration = Duration(duration);
						} catch { throw 'Invalid duration' }
			...
			handler: async ({ user, reason }, { guild, member: author }, { GuildData, reply, error }) =>
				afterMod({ com: 'unban', GuildData, guild, author, user, reason, dmInvite: true, allowNoMember: true },
			...
			handler: async ({ user, reason, duration }, { guild, member: author }, { GuildData, reply, error }) =>
				afterMod({ com: duration ? 'tempmute' : 'mute', GuildData, guild, author, user, reason, duration },
					async ({ member, Moderation }) => {
						...
						if (Moderation.timeout)
							...
							if (duration) {
								let tempIndex = DataBase.temp.length;
								DataBase.temp[tempIndex] = { g: guild.id, m: member.id, type: 'mute', until: Date.now() + (duration * 1000), role: mutedRole.id, key: [author.id, d, (reason || '*').replace(/\W/g, '').substr(0, 20)].join('-') };
			...
			handler: async ({ user, reason }, { guild, member: author }, { GuildData, reply, error }) =>
				afterMod({ com: 'unmute', GuildData, guild, author, user, reason },
					async ({ member, Moderation }) => {
						if (Moderation.timeout)
