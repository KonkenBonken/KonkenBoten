...
export const commands = ({ client, Duration, CleanDate, capital, moment, ParseModLogs, WriteDataBase, ObjectMerge /* DataBase, Cache*/ }) => [{
		com: 'suggest',
...

	{
		...
		handler: async ({ user, reason }, { reply, guild, member: author }, { GuildData }) => {
			...
			//ONLY ON MUTE:			if (guild.me.roles.highest.comparePositionTo(mutedRole) <= 0) return error();
			...
			// dmInvite:true   ON ['tempban', 'kick', 'unban']
			// duration:_   ON TEMP
		...
		handler: async ({ user, reason }, { reply, user: author }, { GuildData }) => {}
	}
];
