...
export default (globals) => {
	...
	return commands = [{
			...
			handler: ({ message, private }, { channel, reply }) => {
				if (message.length > 2000) {
					channel.send(content.substr(0, 2000));
					content = content.substr(2000);
				}
				return channel.send({ content, ephemeral: !!private }).then(() =>
...

		{
			...
			handler: async ({ user, private }, { guild, reply }, { error }) => {
			...
			handler: async ({ private }, { guild, reply }, { GuildData }) => {
			...
			handler: async ({ role, private }, { member, user: author, reply, guild }) => {
